...
{
    return vec2(position.x, position.y - amplitude*0.3*sin(frequency*characterIndex + speed*u_fTime*0.6));
}
...
vec2 shake(vec2 position, float characterIndex, float magnitude, float speed)
{
    ...
    return position + magnitude*merge*(2.0*delta-1.0)*.15;
}
...
float ease_elastic_out(float time)
{
    if (time <= 0.0) return 0.0;
    if (time >= 1.0) return 1.0;
    return pow(2.0, -10.0*time)*sin(2.09439510239*(10.0*time - 0.75)) + 1.0;
}
...
void main()
{
    ...
    if (abs(u_fTypewriterMethod) != 0.0)
    {
        ...
        float fadeAmount = fade(u_fTypewriterWindowArray, u_fTypewriterSmoothness, index , (u_fTypewriterMethod < 0.0)); //0.0 <= BOON <= 1.0

          // fadeAmount = ease_elastic_out(fadeAmount);
          // pos = scale(pos, centre, fadeAmount);
         //  pos = rotate(pos, centre, 32.0*sin(3.14*fadeAmount)); 
